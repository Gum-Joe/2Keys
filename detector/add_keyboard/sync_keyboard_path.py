  ...
  async with aiofiles.open(os.getcwd() + "/config.yml", mode="r") as config_file:
      ...
      try:
        async with aiohttp.ClientSession() as session:
          async with session.post("http://" + config["addresses"]["server"]["ipv4"] + ":" + str(config["addresses"]["server"]["port"]) + UPDATE_KEYBOARD_PATH, data={ "keyboard": name, "path": keyboard_path }) as resp:
            print(resp)
